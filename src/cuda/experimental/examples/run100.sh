#!/usr/bin/env bash
# Example one-shot launcher — TOP-100 fingerprint pool (100 entries, balanced default).
#
# Run from the repo root after `make -j`:
#     bash examples/run100.sh
#
# Re-attach to the running search:
#     tmux attach -t cc_v16_top100
#
# Stop the search:
#     tmux kill-session -t cc_v16_top100
#
# Good balance between hit rate and structural diversity.
set -uo pipefail

POOL_NAME="top100"
POOL_FILE="pools/known_fingerprints.txt"
