#!/usr/bin/env bash
# Example one-shot launcher — EXP-D fingerprint pool (4248 entries, exponent-aware variants).
#
# Run from the repo root after `make -j`:
#     bash examples/run_exp.sh
#
# Re-attach to the running search:
#     tmux attach -t cc_v16_exp_d
#
# Stop the search:
#     tmux kill-session -t cc_v16_exp_d
#
# Default production pool. ~96.78% structural coverage of historical CC10+ corpus.
# Use this for broad CC21-scale long runs.
set -uo pipefail

POOL_NAME="exp_d"
POOL_FILE="pools/known_fingerprints_exp_d.txt"
