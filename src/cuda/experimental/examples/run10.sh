#!/usr/bin/env bash
# Example one-shot launcher — TOP-10 fingerprint pool (10 entries, most-frequent shapes).
#
# Run from the repo root after `make -j`:
#     bash examples/run10.sh
#
# Re-attach to the running search:
#     tmux attach -t cc_v16_top10
#
# Stop the search:
#     tmux kill-session -t cc_v16_top10
#
# The small pool concentrates GPU time on the highest-probability fingerprints —
# best per-GPU-second hit rate, but lower structural diversity.
set -uo pipefail

POOL_NAME="top10"
POOL_FILE="pools/known_fingerprints_top10.txt"
...
[ "$PROVE_THREADS" -gt 24 ] && PROVE_THREADS=24   # diminishing returns past ~24
